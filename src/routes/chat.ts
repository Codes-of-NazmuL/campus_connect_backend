...
router.get('/rooms/:roomId/messages', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const participant = await prisma.chatRoomUser.findUnique({
      where: { userId_chatRoomId: { userId, chatRoomId: req.params.roomId as string } },
    });
    if (!participant) return res.status(403).json({ error: 'Not a participant.' });

    const messages = await prisma.message.findMany({
      where: { chatRoomId: req.params.roomId as string },
      include: { sender: { select: { id: true, name: true } } },
      orderBy: { createdAt: 'desc' },
      take: 100,
    });
...
router.delete('/rooms/:roomId/leave', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const userId = req.user!.id!;
